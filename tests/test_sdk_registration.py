...
class DummyAgent:
    """Simple agent returning a static response."""
...
class EchoCrew:
    """Crew that echoes provided text."""
...
def length_check(text: str, max_length: int = 5) -> str:
    """Return 'ok' if within limit else 'too long'."""
...
def test_invalid_permissions_type() -> None:
    with pytest.raises(ValueError):
        @jarvis_tool(description="bad", permissions="net")  # type: ignore
        def _():
            pass


def test_invalid_permission_item() -> None:
    with pytest.raises(ValueError):
        @jarvis_tool(description="bad", permissions=["net", 1])  # type: ignore
        def _():
            pass
